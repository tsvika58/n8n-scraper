"""
Simplified unit tests for MultimodalContentProcessor
...
class TestDatabaseOperations:
    ...
    def test_store_image_data_failure(self):
        ...
        assert result is not None
        assert result[6] == 0            # success column (False = 0)
        assert result[7] == error        # error_message column
        assert result[3] is None         # ocr_text column
    ...
    def test_store_video_data_success(self):
        ...
        conn = sqlite3.connect(self.temp_db.name)
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM workflow_videos WHERE workflow_id = ? AND video_url = ?", 
                      (workflow_id, video_url))
        result = cursor.fetchone()
        conn.close()
        
        assert result is not None
        assert result[1] == workflow_id  # workflow_id column
        assert result[2] == video_url    # video_url column
        assert result[3] == video_id     # video_id column
        assert result[4] == transcript   # transcript column
        assert result[6] == 1            # success column (True = 1)
        assert result[7] is None         # error_message column
    ...
    def test_store_video_data_failure(self):
        ...
        assert result[6] == 0            # success column (False = 0)
        assert result[7] == error        # error_message column
        assert result[4] is None         # transcript column
...
class TestDataValidation:
    ...
    def test_store_image_data_with_long_content(self):
        ...
        assert result is not None
        assert result[3] == content      # ocr_text column
        assert result[4] == len(content) # text_length column
...
def processor_with_temp_db(temp_db_path):
    ...
    # Create database tables
    from src.database.multimodal_schema import create_multimodal_tables
    create_multimodal_tables(temp_db_path)
...
class TestProcessorWithTempDb:
    ...
    def test_processor_with_temp_db_initialization(self, processor_with_temp_db):
        ...
        assert processor_with_temp_db.headless is True
        assert processor_with_temp_db.timeout == 30000
    ...
    def test_database_operations_with_temp_db(self, processor_with_temp_db):
        ...
        # Verify data was stored
        import sqlite3
        conn = sqlite3.connect(processor_with_temp_db.db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM workflow_images WHERE workflow_id = ? AND image_url = ?", 
                      (workflow_id, image_url))
        result = cursor.fetchone()
