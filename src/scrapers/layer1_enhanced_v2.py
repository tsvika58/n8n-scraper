"""
Layer 1 Enhanced V2 - Clean Workflow Content Extractor

This module extracts ONLY RELEVANT workflow content from n8n.io pages.
It filters out navigation, footers, testimonials, and other non-workflow content.

Improvements over Layer 1.5:
- Stops at content boundaries (before testimonials/footer)
- Excludes "Complete Page Content" section (navigation/cookie junk)
- Better tag/category filtering
- Optimized for clean, usable data

Author: AI Assistant
Date: October 16, 2025
Version: 2.0.0
Purpose: Extract clean workflow content for direct display without post-processing
"""
...
class Layer1EnhancedV2Extractor:
    """
    ...
    async def initialize(self):
        ...
        logger.info("Layer 1 Enhanced V2 extractor initialized")
    ...
    async def cleanup(self):
        ...
        logger.info("Layer 1 Enhanced V2 extractor cleaned up")
    ...
    async def _extract_all_text_content(self, soup: BeautifulSoup, data: Dict):
        """
        Extract all meaningful text content from the page, stopping at irrelevant sections.
        
        STRATEGY (based on analysis of 20 workflows):
        1. Extract only from first 2 <section> elements (sections 0-1)
        2. Section #2 is always "More templates by [Author]" (irrelevant)
        3. Sections #3-6 are navigation/footer (irrelevant)
        4. Also remove <footer> tag if present
        5. Stop at text markers: "More templates by", "Stars", etc.
        """
        ...
        # STRATEGY 1: Find and remove section.section-creator-workflows and everything after it
        creator_section = soup.find('section', class_=lambda x: x and 'section-creator-workflows' in str(x))
        if creator_section:
            # Remove this section and all following siblings
            for sibling in list(creator_section.find_next_siblings()):
                sibling.decompose()
            creator_section.decompose()
            logger.debug("Removed 'section-creator-workflows' and all following sections")
        
        # STRATEGY 2: Remove footer (backup strategy)
        footer = soup.find('footer')
        if footer:
            footer.decompose()
            logger.debug("Removed footer section")
        
        # STRATEGY 3: Keep only first 2 sections if we have 7+ sections
        sections = soup.find_all('section')
        if len(sections) >= 7:
            # Remove sections 2-6 (keep only 0-1)
            for idx, section in enumerate(sections):
                if idx >= 2:
                    section.decompose()
            logger.debug(f"Kept only first 2 sections out of {len(sections)}")
        
        # Get all text from remaining content
        ...
        logger.debug(f"Extracted {len(clean_text)} characters of clean text (irrelevant sections excluded)")
    ...
    def format_as_markdown(self, workflow_id: str, data: Dict) -> str:
        ...
        # REMOVED: Complete page content section (contains navigation/footer junk)
        # This was causing cookie dialogs, testimonials, and footer links to appear
        # The relevant workflow content is already captured in sections above
