#!/usr/bin/env python3
"""
Simple Layer 1 Scraper - Reliable and Monitorable

Features:
- One workflow at a time (no parallel complexity)
...
class SimpleLayer1Scraper:
    """Simple, reliable Layer 1 scraper."""
    ...
    def save_workflow_data(self, workflow_id, data):
        """Save workflow data to database immediately."""
        with get_session() as session:
            try:
    ...
    async def run(self, limit=None):
        ...
        logger.info("="*70)
        logger.info("STARTING SIMPLE LAYER 1 SCRAPING")
        logger.info("="*70)
...
async def main():
    """Main entry point."""
    scraper = SimpleLayer1Scraper()
    
    # Start with a small batch to test
    logger.info("Starting with first 10 workflows for testing...")
    ...
    logger.info("python /app/scripts/simple_layer1_scraper.py --all")
