#!/usr/bin/env python3
"""
SCRAPE-015: Test 10 Real Workflows

Tests the complete system by scraping 10 unscraped workflows:
1. Selects 10 workflows that haven't been scraped yet
2. Runs real-time scraping with monitoring
3. Updates database in real-time
4. Shows live progress on dashboard
"""
    ...
    try:
        ...
        for line in lines:
            if '|' in line:
                ...
                url = parts[1].strip()
                workflows.append((workflow_id, url))
        ...
        for i, (workflow_id, url) in enumerate(workflows, 1):
            print(f"🔍 [{i}/10] Scraping workflow {workflow_id}...")
            print(f"   URL: {url}")
