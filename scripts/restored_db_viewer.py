#!/usr/bin/env python3
"""
Restored Database Viewer - Original Beautiful Implementation
Based on commit e9938c5: "✨ Add interactive database viewer with clickable workflow details"
"""
    ...
    try:
        ...
        query = """
            SELECT 
                ...
                ROUND(AVG(w.quality_score)::numeric, 2) as avg_quality_score,
                ROUND(AVG(w.processing_time)::numeric, 2) as avg_processing_time
            FROM workflows w
        ...
        return {
            ...
            'layer1_complete': int(stats['layer1_complete']),
            'layer2_complete': int(stats['layer2_complete']),
            'layer3_complete': int(stats['layer3_complete']),
            ...
            'success_rate': round((stats['layer1_complete'] / stats['total_workflows'] * 100), 1) if stats['total_workflows'] > 0 else 0
        }
...
class DBViewerHandler(BaseHTTPRequestHandler):
    ...
    def serve_dashboard(self):
        ...
        stats = get_statistics()
        workflows, total = get_workflows(limit=50, offset=0)
    ...
    <div class="container">
        ...
        <div class="stats">
            <div class="stat-card">
                ...
                <div class="value" id="total-workflows">{stats['total_workflows']}</div>
            </div>
            <div class="stat-card">
                <div class="label">Layer 1 (Metadata)</div>
                <div class="value" id="layer1-complete">{stats['layer1_complete']}</div>
            </div>
            <div class="stat-card">
                <div class="label">Layer 2 (Structure)</div>
                <div class="value" id="layer2-complete">{stats['layer2_complete']}</div>
            </div>
            <div class="stat-card">
                <div class="label">Layer 3 (Media)</div>
                <div class="value" id="layer3-complete">{stats['layer3_complete']}</div>
            </div>
            <div class="stat-card">
                <div class="label">Completion Rate</div>
                <div class="value" id="success-rate">{stats['success_rate']}%</div>
            </div>
        </div>
