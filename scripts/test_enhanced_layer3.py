"""
Test Enhanced Layer 3: Comprehensive Multimedia Discovery

Tests the enhanced Layer 3 extractor on video-rich workflows to validate
100% multimedia discovery capabilities.
...
async def test_enhanced_layer3():
    """Test enhanced Layer 3 on video workflows"""
    
    print('🎥 TESTING ENHANCED LAYER 3 - COMPREHENSIVE MULTIMEDIA DISCOVERY')
    print('=' * 80)
    print()
    
    # Test workflows with videos (based on user feedback)
    video_workflows = [
    ...
    async with EnhancedLayer3ExplainerExtractor() as extractor:
        for wf_id, url in video_workflows:
    ...
    avg_time = sum(r.get("extraction_time", 0) for r in successful_tests) / len(successful_tests) if successful_tests else 0
    ...
    results_file = Path(__file__).parent / 'enhanced_layer3_test_results.json'
...
if __name__ == "__main__":
    success = asyncio.run(test_enhanced_layer3())
