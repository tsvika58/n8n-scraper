...
def clear_screen():
    """Clear screen - works in both interactive and non-interactive terminals"""
...
def monitor_all_layers():
    ...
    try:
        while True:
            ...
            # Print separator for new update
            print("\n" + "─" * 80)
            print(f"📊 UPDATE #{iteration} - {now_jst.strftime('%Y-%m-%d %H:%M:%S')} Jerusalem Time")
            print("─" * 80)
            ...
            if l1_5['completed'] > 0 and l2['completed'] > 0:
                l1_5_finish_time = now + timedelta(seconds=(l1_5['remaining'] * l1_5_elapsed / l1_5['completed']))
                l2_finish_time = now + timedelta(seconds=(l2['remaining'] * l2_elapsed / l2['completed']))
                
                if l1_5['remaining'] > 0 and l2['remaining'] > 0:
                    if l1_5_finish_time < l2_finish_time:
                        diff = format_time((l2_finish_time - l1_5_finish_time).total_seconds())
                        print(f"\n🏁 Layer 1.5 will finish first (~{l1_5_eta_str})")
                        print(f"   Layer 2 will finish ~{diff} later")
                    else:
                        diff = format_time((l1_5_finish_time - l2_finish_time).total_seconds())
                        print(f"\n🏁 Layer 2 will finish first (~{l2_eta_str})")
                        print(f"   Layer 1.5 will finish ~{diff} later")
