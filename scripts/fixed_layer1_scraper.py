#!/usr/bin/env python3
"""
FIXED Layer 1 Scraper - No SQL Syntax Errors

This version uses consistent SQLAlchemy parameter binding.
"""
...
class FixedLayer1Scraper:
    """Fixed Layer 1 scraper with proper SQL syntax."""
    ...
    async def scrape_and_save_workflow(self, workflow_id, url):
        """Scrape one workflow and save immediately."""
        try:
    ...
    def save_workflow_data(self, workflow_id, data):
        """Save workflow data to database immediately - FIXED SQL SYNTAX."""
        with get_session() as session:
            try:
                # Use consistent :parameter style throughout
    ...
    async def run(self, limit=None):
        ...
        logger.info("="*70)
        logger.info("STARTING FIXED LAYER 1 SCRAPING")
        logger.info("="*70)
...
async def main():
    ...
    logger.info("python /app/scripts/fixed_layer1_scraper.py --all")
