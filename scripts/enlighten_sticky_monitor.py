#!/usr/bin/env python3
"""
ENLIGHTEN STICKY MONITOR - Uses enlighten library for proper sticky progress bar
"""
import asyncio
import time
import sys
...
async def main():
    """Main scraping function with enlighten sticky progress bar"""
    ...
    # Create progress bar with enlighten
    ...
    # Create status bar for additional info
    ...
    try:
        ...
        for i, workflow_id in enumerate(workflows, 1):
            ...
            if completed > 0 and elapsed > 0:
                ...
                eta_seconds = (total_workflows - completed) / rate if rate > 0 else 0
                eta_str = format_time(eta_seconds)
            else:
                eta_str = "Calculating..."
            
            status_text = f"Current: {workflow_id} | Done: {completed}/{total_workflows} | Failed: {failed} | ETA: {eta_str} | {jerusalem_time}"
            status_bar.update(status=status_text)
